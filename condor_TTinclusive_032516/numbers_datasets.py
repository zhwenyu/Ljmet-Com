...
for sample in samplelist:
    print " "
    print 'looking up details for the following dataset(s): ',sample
    os.system('python -u das_CLI.py --limit=0 --query="dataset = '+sample+'" --format=JSON --das-headers >& datasetDetails/'+sample.split('/')[1]+'_'+sample.split('/')[2]+'_numbers.json')
    with open('temp/'+sample.split('/')[1]+'_'+sample.split('/')[2]+'_numbers.json') as data_file:    
