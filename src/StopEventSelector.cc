...
private:
  ...
  std::string GetTriggerName(std::string channel, 
			     bool isMc,  bool isSigMc,
			     int run);
...
void StopEventSelector::BeginJob( std::map<std::string, edm::ParameterSet const> par){
  ...
  _key = "event_selector";
  if ( par.find(_key)!=par.end() ){
    mbPar["is_signal_mc"]             = par[_key].getParameter<bool>         ("is_signal_mc");
...
void StopEventSelector::AnalyzeEvent( edm::EventBase const & event,
  ...
  // standard composite trigger branches
  int _run   = event.id().run();
  bool _mujets = false;
  std::string _mujets_name = GetTriggerName("mujets", _mbIsMc, 
					    mbPar["is_signal_mc"],
					    _run);
  bool _mujets2 = false;
  std::string _mujets2_name = GetTriggerName("mujets2", _mbIsMc, 
					    mbPar["is_signal_mc"],
					    _run);
  ...
  for (unsigned int i=0; i<_tSize; i++){
    ...
    bool fired = mhEdmTriggerResults->accept(trigNames.triggerIndex(trigName));
    //std::cout <<", FIRED = "<<fired<<std::endl;
    ec.SetValue("trig_"+_sName, fired);
    ec.SetValue("trig_"+_sName+"_ver", _version);

    
    // standard composite trigger branches
    if (_sName==_mujets_name) _mujets = fired;
    if (_sName==_mujets2_name) _mujets2 = fired;

  } 


  // standard composite trigger branches
  ec.SetValue("trigger_mujets_TOPPAG", _mujets);
  ec.SetValue("trigger_mujets2_TOPPAG", _mujets2);
...
std::string
StopEventSelector::GetTriggerName(std::string channel,
				  bool isMc, bool isSigMc,
				  int run){
  //
  // returns the name of trigger from TOP PAG 
  // given data-MC and run number
  //
  ...
  if (channel == "mujets"){
    if (isMc){
      if (isSigMc) tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet45_35_25"; // _v1 truncated;
      else tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet50_40_30";
    }
    else{
      if ( run >= 190456 && run <= 193805 ) tName = "HLT_IsoMu17_eta2p1_TriCentralPFJet30";
      if ( run >= 193806 && run <= 198021 ) tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet30_30_20";
      if ( run >= 198022 && run <= 999999 ) tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet45_35_25";
    }
  }

  else if (channel == "mujets2"){
    if (isMc){
      if (isSigMc) tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet45_35_25";
      else tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet50_40_30";
    }
    else{
      if ( run >= 190456 && run <= 193805 ) tName = "HLT_IsoMu17_eta2p1_TriCentralPFJet30";
      if ( run >= 193806 && run <= 198021 ) tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet30";
      if ( run >= 198022 && run <= 999999 ) tName = "HLT_IsoMu17_eta2p1_TriCentralPFNoPUJet30_30_20";
    }
  }
