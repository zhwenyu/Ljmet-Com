...
#include <iostream>
#include <stdexcept>
#include <math.h>  /* fabs, pow, sqrt, cos */
...
int LJetsTopoVars::setEvent(std::vector<TLorentzVector> & jets,
    ...
    double C = std::max(1. + pow(nu_e,2) * (pow(l_pz,2)-pow(l_e,2)) / pow(A+B,2)  , 0.);
    C = sqrt(C);
...
double LJetsTopoVars::maxJetEta() const
{
    ...
    for (unsigned int i=0; i<m_jets.size(); i++) {
        if(fabs(m_jets.at(i).Eta()) > fabs(jetEta) ) jetEta = fabs(m_jets.at(i).Eta());
    }
    return jetEta;
}
...
double LJetsTopoVars::Jet1Jet2_DeltaPhi() {
    if(m_jets.size()>=2) {
        return fabs(m_jets.at(0).DeltaPhi(m_jets.at(1)));
    } else return -1;
}
...
void LJetsTopoVars::calcHt(){
    ...
    for(int i=0;i<nJet;i++){
        ...
        if(fabs(m_jets[i].Eta())>_ht[11] && i<4){
            _ht[11] = fabs(m_jets[i].Eta());
        }
        for(int j=i+1; j<nJet; j++){
    ...
    // the "M_T"s
    if(mtjets > 0.){ _ht[13]=sqrt(mtjets); }
    _ht[14] = _ht[0]*_ht[0] - hx*hx - hy*hy;
    if(_ht[14]>0.) _ht[14] = sqrt(_ht[14]);
    _ht[15] = h*h - hzSigned*hzSigned;
    if(_ht[15]>0.) _ht[15] = sqrt(_ht[15]);
    
    if(_ht[0]>0.) _ht[16] = hzSigned/_ht[0];
    
    double hzNoLep = hz;
    hz += fabs(m_lepton.Pz());
    hz += fabs(_neutrino.Pz());
    if(hz!=0.){
        _ht[1]=_ht[0]/hz;
        _ht[4]=_ht[3]/hz;
        _ht[7]=_ht[6]/hz;
...
void LJetsTopoVars::calcMt()
{
    ...
    // might be scaled???
    //
    double met = sqrt(pow(_neutrino.Px(),2.)+
                             pow(_neutrino.Py(),2));
    
    
    _mt[0] = fabs(m_lepton.DeltaPhi(_neutrino));
    _mt[1] = sqrt(2*m_lepton.Pt()*met*(1.-cos(_mt[0])));
